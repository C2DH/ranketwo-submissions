...
### What are data?
The word "data" comes from Latin. The verb "dare" means "to give," so "datum" means "that which is given" as in the French term "données" (https://projects.iq.harvard.edu/files/eswg/files/rosenburg_-_rawdata.pdf) Apart from the meaning of data in the digital realm, we still use the term to refer to specific days, months, and years. This means that data can be both digital and non-digital. Just as there are non-digital calendars, there are also non-digital data. Let's start by understanding what "data" means and what its characteristics are. Then, we'll explore the specifics of digital data.
...
As you see, the punchard can't hold much information, something around 60 bytes (one byte is 8 bits long, one bit being either a `0` or a `1`). You can play around with punchcards and see for yourself what inforation you can fit onto one of them. 

#### Assignment
The Austrian artist and web-designer Norbert Landsteiner has build a [beautiful emulator](https://masswerk.at/keypunch/) to further explore this medium. Give it a try, punch a short data-set describing yourself or a person you know well.
...
To get a digital (= numerical and binary) representation of charcaters and text, we need to agree on a scheme (=encoding) that specifies which number represents which character, very much as the Morse-code did. EBCDIC was one such a scheme. It assigned numbers from 0 to 255 (the largest number you can represent in 8 bits) to some common characters and symbols. We can imagine this as a 16 x 16 grid, where each symbol or character is represented by a specific position on the coordinate-grit, respectively its numerical value. 

Below is a table showing you the numerical value (in grey) of these characters, as well as their binary expression. The fields in red are control-characters. You might notice the little italic letters in blue and the row and column headers in dark blue. These are the hexadecimal representations of binarys. Early in the history of information technology, programmers came up with this way to represent binary numbers, because it's much more easy for humans to read them. As a 16-base system, you basically count normally from 0 to 9, but continue with the letters A to F to represent the numbers from 10 up to 16. For example, `A` = 10, `B` = 11, `C` = 12, and so on. The trick with hexadecimal numbers is that you need just two symbols to represent one byte (i.e. any number between 0 and 255). As you see in the table, you can for example refer to the decimal number 188 with the hexadecimal term `BC`, or to number 123 with the hex-code `7B`. 
We can now interpret the coordinate system we see below. For example, the letter **A** in caps is in field `C-1`, which corresponds with the binary number `11000001` or `193` in the decimal system.[^1]

![EBCDIC table](/images/EBCDIC_table.jpg)

[^1]: If you want to see how exactly the EBCDIC character-codes were mapped on the IBM-punchcards, check out the insightful resources on punchcard history assembeld by Douglas W. Jones: https://homepage.cs.uiowa.edu/~dwjones/

Another very old way to represent text is the ASCII encoding. ASCII uses only 7 bits and can therefore represent 128 different characters, 95 of which are used for the most common letters and special characters. You can see the mapping of characters to numbers in this table:

![ASCII table](/images/ASCII_table.jpg)

Here, too, numbers are assigned to the letters, just as in the EBCDIC coordinate system above. Each letter or character is assigned a number between 0 and 127. For example, the letter **A** (capital) is assigned the number `65`. As you already know, we don't use the decimal system (0 to 9) to express numbers in bit encoding, but instead use the binary system, using just the two digits `0` and `1`. So the numerical value `65` of our letter **A** is expressed as the binary value `01000001` (or `41` in hexadecimal, if you prefer). Therefore, a longer text in ASCII encoding is ultimately a long chain of ones and zeros. The sentence "What are data?" is represented as a sequence of 1s and 0s in ASCII: `01010111 01101000 01100001 01110100 00100000 01100001 01110010 01100101 00100000 01100100 01100001 01110100 01100001 00111111`.[^2]

[^2]: Many web-based converters allow you to switch from ASCII to binary and back, for example this one: https://www.rapidtables.com/convert/number/ascii-to-binary.html
...
This principle of representing single pixels with numeric values is also how 8-bit grayscale works. In this scheme, there are 256 shades of grey between white and black. To implement this, just imagine that each field in our 16x16 grid has now the numerical code for a specific shade of gray assigned to it. 
Colors can also be represented in this way, using the 8-bit [RGB scheme](https://www.rapidtables.com/web/color/RGB_Color.html), which recognizes 256 different combinations of the primary colors red, green, and blue. Of course, the bigger our grid get's, the higher the resolution of our image will be!
...
This table now describes a person, listing some of the attributes and the corresponding values we might consider useful for identifying it. Obviously, our choice of attributes would be different if we wanted to describe something else, like a place, or a piece of art, or an animal, or a book. This secelction is called *siginficant properties*, because, for identifying a fugitive criminal, for example, her or his favorite dessert might not be the most significant identifyer, especially if we can list only a limited number of them on our wanted-poster. 

But what woud we do, if we wanted to collect information on several historic persons? If you imagine tilting our two-column table to the right, the attributes become column headers and we can add more rows to it, each one describing one person:
...
You have already got to know JSON. It is widely used for annotation and transfer in the humanities because it is easily readable and and customizable. It can be used to describe all sorts of objects/entities, including historic persons. Below is a short dataset describing our old friend Karl in JSON:
```json
{
...
But how do you know if you produceed valid and pretty JSON? Just copy it and paste it into one of the many online checkers, like [this one](https://jsonchecker.com/). There many others out there that help you correct your errors, but before you use [one of them](https://jsonformatter.curiousconcept.com/), try to spot your error yourself!

---

What have we learned? Data can be used to describe all kinds of objects and phenomena. we want our description to be precise and effective, so we need to think about the *significant properties* (=attributes) we want to capture in our description/table. If we want to have tables longer than just one row, these attributes or properties have to be selected in a way that makes sense for all of the phenomena, objects, people, or whatever else we want to describe in our table. In addition, we need to consider the *data_types*: the vaules we assign to each of the attributes in our table have to be coherent. For our data to be processable by computers and software, it needs to adhere to a pre-defined *data schema*, like the examples we've encountered with JSON.

 ### Databases
We have seen that we can store all kinds of information in tables. For a comprehensive and critical analysis, we want to capture and study our sources from many different angles, collecting a great variety of information. However, coherence and clarity would get lost if we would stuff everything in one single table. But if we create many tables, how can we discover relations then? Because, after all, we're interested in discovering in connections, patterns, and correlations. Realtional databases allow us to do precisely that! They are capable of storing a great variety of different data and their respective database management systems (DMBS) enable us to perform the basic sorting, filtering and searching operations that build the foundation of research and analysis.
...
![Source: "Whos Atrocities?"](/images/source_full.jpg)

First of all, we notice that we could obviously treat this source on three different levels: On the one hand, we're dealing with a website. On a different level, it is an excerpt from an edited volume assembling Marx' writings for the New York Daily Tribune that was published in 1951. Finally, we could also treat it as an article that appeard in said newspaper on April 10, 1857. This is how these different types are displayed in zotero[^6]:

![zotero entries](/images/zotero_entries.jpg)

The same source would look like this if captured in tropy:
![tropy_screen](/images/tropy_scree.jpg)

[^6]: Zotero is a very useful and popular software for manging bibliographic information, basically a database. It can import and export all of the most common data-formats for bibliographic data. (If you do not yet use zotero, head to its [webpage](https://www.zotero.org/) and download the appropriate version for your system.) You can see that zotero let's you enter additional information on your source, depending on how you want to treat (and further analyse) it: website, chapter in an edited volume, or newspaper article. You can also attach keywords ("tags") that will make it easier to create collections of sources on similiar subjects. A similar useful software is [tropy](https://www.tropy.org/). It is designed to help you organize your research findings, particularly fotos. It is enormously helpful if you work in an archive and take pictures of historical records and documents that you want to analyze and sort later on.

Our job here is to capture all names of people, places, dates, and events in this source. This process can be automated to a certain extent and is called "Named Entity Recognition" (NER). We do it by hand. Of course we could store the entities in the paragraph just in one spreadsheet, but let's imagine we will extract information from many different sources or an entire corpus (for example all of Marx' writings, or all English newspaper articles on China concerning the Opium Wars). If we just have a look on the first few sentences, we encounter already a lot of information.

![first paragraphs](/images/source_zoom.jpg)

We find several names, each with an office or title, the respective institutions, as well as references to historical events yet to be clarified. In addition, we could look up all these enteties and acquire additional information on dates and times and various places. This is a lot of onformation already. If we wanted to stuff it all in one single table, it would become huge and difficult to work with. that's why we want a relational database.
...
In a relational database, Palmerston exists **exactly once** as row `P002`. Every event, every office, every source that mentions him simply stores `P002`. You can then ask questions that cut across all your sources at once: *"List every event involving persons who held office in the British Government, sorted by date."* A spreadsheet cannot answer that. A relational database answers it in a single query, however many sources you have gathered.

For a relational database to work, it is indespensable that every row has a **unique identifier** - usually the first column - so the database can tell records (the rows) apart without ambiguity. Imagine two people are both named "John Smith" in your sources — or that Lord Palmerston is referred to as "the Premier", "his Lordship", and "Lord Palmerston" in three different paragraphs. A **primary key** (PK) solves this: it is a short code (`P001`, `P002` …) that belongs to one and only one row, permanently. You invent it yourself when building the database, or the software you use assigns it autmatically to each row/record.
...
*Note that we only see the first three enteties here, further persons in the excerpt to be included in this table are: Harry Parkes (the Consul), Sir John Bowring, Admiral Seymour …*
...
|---|---|---|---|---|---|
| OF001 | P001 | O001 | Director | *unknown* | *unknown* |
| OF002 | P002 | O002 | Premier of England | *before 1857* | *after 1857* |
| OF003 | P003 | O002 | Minister of Foreign Affairs | *before 1857* | *after 1857* |
...
#### Table 6 — Events *(the connecting hub)*
...
| E003 | Arrest of sailors aboard the *Arrow* | 1856-10-08 | L003 | O001 | S001 |

#### Table 7 — Person–Event *(the junction table)*
...
Of course, there are rules to follow when creating a database. Primary keys cannot be empty (null), for a start. Also, as we've aldready seen for other data-sets, the entries (for each column) have to be of the same type. If you use a software to create a database (like the free [Libre Office Base](https://www.libreoffice.org/DISCOVER/BASE/)), you will have to specify the types. Also, the software will ask you whether a column can hold foeign keys or not (you usually recognize them , because they are IDs, i.e. by their suffix `_id`).

![Types](/images/odb.jpg) 

#### Assignment
You see that Karl mentions an incident that happended on a ship, the Arrow. head over to the respecitve [wikipedia entry](https://en.wikipedia.org/wiki/Second_Opium_War) to get details. Now update the tables, specifically the "events" table. Do we need more tables for this?

---

### What have we learned?
Databases are the main carrier for dgital data. They can store a great variety of different types of data. But their true vaule comes form their ability to run queries over all of their content. This way, we can get very specific information, evaluate the magnitude or duration of phenomena and detect patterns in the data. 

### Reading/viewing suggestions
- Octave Julien. Creating and using databases in medieval history: historiography, concepts and prac-
tice. Seminários do GIHM, Apr 2024, Porto, Portugal. ￿halshs-04550456￿
